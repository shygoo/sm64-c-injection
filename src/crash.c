...
{
    u8 *mem = (u8 *) 0x80000000;
    int offs = 0;

    for(int i = 0; i < 2; i++)
    {
        *(u32 *) &mem[offs+0x00] = 0x08000000 | (((u32)newEntry & 0x03FFFFFF) >> 2);
        *(u32 *) &mem[offs+0x04] = 0x00000000;
        *(u32 *) &mem[offs+0x08] = 0x08000000 | (((u32)origEntry & 0x03FFFFFF) >> 2);
